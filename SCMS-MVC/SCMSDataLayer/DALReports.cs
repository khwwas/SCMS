    ...
    public class DALReports
    {
        ...
        public DataSet BudgetSummary(string ps_Location, int pi_AllYear, string ps_Year)
        {
            ...
            try
            {
                ...
                _Sql += "                   SYSTEM_Nature, ";
                _Sql += "                   SETUP_ChartOfAccount ";
                _Sql += "             Where ( SYSTEM_Nature.Natr_SystemTitle In ( 'Expense', 'Assets' ) ) And ";
                _Sql += "                   ( GL_VchrMaster.VchMas_Status = 'Approved' ) And ";
                _Sql += "                   ( GL_VchrMaster.Loc_Id = SETUP_Location.Loc_Id ) And ";
        ...
        public DataSet BudgetAndMonthlyExpense(string ps_Location, int pi_AllYear, string ps_Year)
        {
            ...
            try
            {
                ...
                _Sql += "          SETUP_ChartOfAccount.ChrtAcc_Title, ";
                _Sql += "          SETUP_ChartOfAccount.ChrtAcc_Type, ";
                _Sql += "          SETUP_Location.Loc_Id, ";
                _Sql += "          SETUP_Location.Loc_Title, ";
                _Sql += "          SETUP_Calendar.Cldr_Title, ";
                _Sql += "          SETUP_Calendar.Cldr_DateStart, ";
                _Sql += "          ActualBudget.BgdtDet_TotalAmount, ";
                _Sql += "          ( Select SUM( ISNULL( GL_VchrDetail.VchMas_DrAmount, 0 ) ) ";
                _Sql += "              From GL_VchrMaster,  ";
                _Sql += "                   GL_VchrDetail ";
                _Sql += "             Where ( GL_VchrMaster.VchMas_Status = 'Approved' ) And ";
                _Sql += "                   ( GL_VchrDetail.VchMas_Id = GL_VchrMaster.VchMas_Id ) And  ";
                _Sql += "                   ( GL_VchrDetail.ChrtAcc_Id = SETUP_ChartOfAccount.ChrtAcc_Id ) And  ";
                _Sql += "                   ( SETUP_ChartOfAccount.Natr_Id = SYSTEM_Nature.Natr_Id ) And  ";
                _Sql += "                   ( Month( GL_VchrMaster.VchMas_Date ) = Month( GetDate() ) )  ";
                _Sql += "          ) As ThisMonthExpense, ";
                _Sql += "          ( Select SUM( ISNULL( GL_VchrDetail.VchMas_DrAmount, 0 ) )  ";
                _Sql += "              From GL_VchrMaster,  ";
                _Sql += "                   GL_VchrDetail  ";
                _Sql += "             Where ( GL_VchrMaster.VchMas_Status = 'Approved' ) And ";
                ...
                _Sql += "                   ( GL_VchrDetail.ChrtAcc_Id = SETUP_ChartOfAccount.ChrtAcc_Id ) And  ";
                _Sql += "                   ( SETUP_ChartOfAccount.Natr_Id = SYSTEM_Nature.Natr_Id ) And  ";
                _Sql += "                   ( GL_VchrMaster.VchMas_Date >= SETUP_Calendar.Cldr_DateStart And  ";
                _Sql += "                     GL_VchrMaster.VchMas_Date <= CONVERT(VARCHAR(25),DATEADD(dd,-(DAY(GETDATE())),GETDATE()),101) )  ";
                _Sql += "          ) As PreviousMonthExpense  ";
                _Sql += "     From SETUP_ChartOfAccount LEFT OUTER JOIN SETUP_Location On ";
                _Sql += "               ( SETUP_Location.Loc_Id IN ( " + ps_Location + " )  ) ";
                _Sql += "          LEFT OUTER JOIN SETUP_Calendar On ";
                _Sql += "               ( SETUP_Calendar.Cldr_Id = '" + ps_Year + "' ) ";
                _Sql += "          LEFT OUTER JOIN ( Select GL_BgdtDetail.ChrtAcc_Id, ";
                _Sql += "                                   GL_BgdtMaster.Loc_Id, ";
                _Sql += "                                   GL_BgdtMaster.Cldr_Id, ";
                _Sql += "                                   GL_BgdtDetail.BgdtDet_TotalAmount ";
                _Sql += "                              From GL_BgdtMaster,  ";
                _Sql += "                                   GL_BgdtDetail ";
                _Sql += "                             Where ( GL_BgdtDetail.BgdtMas_Id = GL_BgdtMaster.BgdtMas_Id ) ) As ActualBudget ON ";
                _Sql += "               ( ActualBudget.ChrtAcc_Id = SETUP_ChartOfAccount.ChrtAcc_Id ) And ";
                _Sql += "               ( ActualBudget.Loc_Id = SETUP_Location.Loc_Id ) And  ";
                _Sql += "               ( ActualBudget.Cldr_Id = SETUP_Calendar.Cldr_Id ), ";
                _Sql += "          SYSTEM_Nature ";
                _Sql += "    Where ( SYSTEM_Nature.Natr_SystemTitle In ( 'Expense', 'Assets' ) ) And ";
                _Sql += "          ( SETUP_ChartOfAccount.Natr_Id = SYSTEM_Nature.Natr_Id ) ";
                _Sql += " Order BY SETUP_Location.Loc_Title, ";
