...
<table id="CityGrid" class="data display datatable">
    ...
    <tbody>
        <% 
            ...
            var lList_Data = new SCMSDataLayer.DALDepartment().GetAllRecords();
            ...
            {
                foreach (SCMSDataLayer.DB.SETUP_Department lRow_Data in lList_Data)
                {
        ...
        <tr class='odd gradeX' style='line-height: 15px;'>
            <td>
                <div onclick="javascript:return EditRecord('<%=lRow_Data.Dpt_Id%>')" style="width: 22px;
                    padding-right: 5px; float: left; cursor: pointer;">
                    <img alt="Edit" src="../../img/edit.png" style="width: 22px; vertical-align: middle" />
                </div>
                <div onclick="javascript:return DeleteRecord('<%=lRow_Data.Dpt_Id%>')" style="width: 22px;
                    float: left; cursor: pointer;">
                    <img alt="Delete" src="../../img/delete.png" style="width: 22px; vertical-align: middle" />
                </div>
            </td>
             <td id="txt_Title<%=lRow_Data.Dpt_Id%>" style="vertical-align: middle;">
                <%=lRow_Data.Dpt_Title%>
            </td>
            <td id="ddl_location<%=lRow_Data.Dpt_Id%>" style="vertical-align: middle;">
            <%if (LocationRow != null)
                  { %>
