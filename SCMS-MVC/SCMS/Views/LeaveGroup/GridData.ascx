...
<table id="CityGrid" class="data display datatable">
    ...
    <tbody>
        <% 
            ...
            var lList_Data = new SCMSDataLayer.DALLeaveGroup().GetAllRecords(); ;
            ...
            {
                foreach (SCMSDataLayer.DB.SETUP_LeaveGroup lRow_Data in lList_Data)
                {
        ...
        <tr class='odd gradeX' style='line-height: 15px;'>
            <td>
                <div onclick="javascript:return EditRecord('<%=lRow_Data.LevGrp_Id%>')" style="width: 22px;
                    padding-right: 5px; float: left; cursor: pointer;">
                    <img alt="Edit" src="../../img/edit.png" style="width: 22px; vertical-align: middle" />
                </div>
                <div onclick="javascript:return DeleteRecord('<%=lRow_Data.LevGrp_Id%>')" style="width: 22px;
                    float: left; cursor: pointer;">
                    <img alt="Delete" src="../../img/delete.png" style="width: 22px; vertical-align: middle" />
                </div>
            </td>
             <td id="txt_Title<%=lRow_Data.LevGrp_Id%>" style="vertical-align: middle;">
                <%=lRow_Data.LevGrp_Title%>
            </td>
            <td id="ddl_location<%=lRow_Data.LevGrp_Id%>" style="vertical-align: middle;">
            <%if (LocationRow != null)
                  { %>
