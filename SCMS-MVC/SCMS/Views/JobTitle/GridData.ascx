...
<table id="CityGrid" class="data display datatable">
    ...
    <tbody>
        <% 
            ...
            var lList_Data = new SCMSDataLayer.DALJobTitle().GetAllRecords();
            ...
            {
                foreach (SCMSDataLayer.DB.SETUP_JobTitle lRow_Data in lList_Data)
                {
        ...
        <tr class='odd gradeX' style='line-height: 15px;'>
            <td>
                <div onclick="javascript:return EditRecord('<%=lRow_Data.JT_Id%>')" style="width: 22px;
                    padding-right: 5px; float: left; cursor: pointer;">
                    <img alt="Edit" src="../../img/edit.png" style="width: 22px; vertical-align: middle" />
                </div>
                <div onclick="javascript:return DeleteRecord('<%=lRow_Data.JT_Id%>')" style="width: 22px;
                    float: left; cursor: pointer;">
                    <img alt="Delete" src="../../img/delete.png" style="width: 22px; vertical-align: middle" />
                </div>
            </td>
             <td id="txt_Title<%=lRow_Data.JT_Id%>" style="vertical-align: middle;">
                <%=lRow_Data.JT_Title%>
            </td>
            <td id="ddl_location<%=lRow_Data.JT_Id%>" style="vertical-align: middle;">
            <%if (LocationRow != null)
                  { %>
